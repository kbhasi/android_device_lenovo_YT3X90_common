...
#define VIDEO_PLAYBACK_FORMAT HAL_PIXEL_FORMAT_NV12_Y_TILED_INTEL
#define VIDEODROP_DEFAULT_TOLERABLE_NSEC (5000000LL) // 5ms by default
#define VIDEODROP_MAX_TOLERABLE_NSEC (1000000000LL/60) // equals 1 vsync
#endif
    ...
    sp<IProducerListener> listener;
    {
        ...
        if (expectedPresent != 0) {
            ...
            while (mCore->mQueue.size() > 1 && !mCore->mQueue[0].mIsAutoTimestamp) {
                ...
                long long int tolerable_nsecs = 0LL;
#ifdef GFX_GEN
                //
                // IMINAN-21143
                // Relax the video drop rule a bit since sometimes queueBuffer()
                // is called a bit early due to A/V sync.
                if (bufferItem.mGraphicBuffer->getPixelFormat() == VIDEO_PLAYBACK_FORMAT)
                    tolerable_nsecs = property_get_int64("videodrop.nsec.override",
                             VIDEODROP_DEFAULT_TOLERABLE_NSEC);
                // don't relax too much
                if (tolerable_nsecs > VIDEODROP_MAX_TOLERABLE_NSEC)
                    tolerable_nsecs = VIDEODROP_MAX_TOLERABLE_NSEC;
#endif
                if (desiredPresent < expectedPresent - MAX_REASONABLE_NSEC ||
                        desiredPresent > expectedPresent - tolerable_nsecs) {
