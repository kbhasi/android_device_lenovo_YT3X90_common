...
status_t Composer::setFlags(const sp<SurfaceComposerClient>& client,
    ...
    if (!s)
        return BAD_INDEX;
    /* BLADEIII, liujk2, add for dlp extension mode, begin @{ */
#ifdef DLP_FEATURE_EXTENSION_MODE
    if (mask & ISurfaceComposerClient::eProjectorable) {
        s->what |= layer_state_t::eProjectorableChanged;
        if (flags & ISurfaceComposerClient::eProjectorable){
            s->projectorable=true;
        }else{
            s->projectorable=false;
        }
    } else if (mask & ISurfaceComposerClient::eProjectedMain) {
        s->what |= layer_state_t::eProjectedMainChanged;
        if (flags & ISurfaceComposerClient::eProjectedMain){
            s->projected_main=true;
        }else{
            s->projected_main=false;
        }
    } else {
#endif
    /* BLADEIII, liujk2, add for dlp extension mode, end @} */
