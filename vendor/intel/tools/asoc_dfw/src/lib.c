	...
	const struct snd_kcontrol_new *kcontrol)
{
	...
	mc = calloc(1, sizeof(*mc) + mixer->pvt_data_len);
	if (!mc)
		return -ENOMEM;
	...
	mc->pvt_data_len  = mixer->pvt_data_len;
	if (mc->pvt_data_len)
		memcpy(mc->pvt_data, mixer->pvt_data, mixer->pvt_data_len);
	...
	bytes = write(soc_fw->out_fd, mc, sizeof(*mc)+ mc->pvt_data_len);
	if (bytes != sizeof(*mc)+ mc->pvt_data_len) {
	...
	const struct snd_kcontrol_new *kcontrol)
{
	...
	ec = calloc(1, sizeof(*ec) + menum->pvt_data_len);
	if (!ec)
		return -ENOMEM;
	...
	verbose(soc_fw, " enum: \"%s\" R1/2 0x%x shift L/R %d/%d (g,p,i) %d:%d:%d\n len =%d\n",
		ec->hdr.name, ec->reg, ec->shift_l, ec->shift_r,
		SOC_CONTROL_GET_ID_GET(ec->hdr.index),
		SOC_CONTROL_GET_ID_PUT(ec->hdr.index),
		SOC_CONTROL_GET_ID_INFO(ec->hdr.index), ec->pvt_data_len);

	import_enum_control_data(soc_fw, ec->items, ec, menum);

	ec->pvt_data_len = menum->pvt_data_len;
	if (ec->pvt_data_len) {
		memcpy(ec->pvt_data, menum->pvt_data, menum->pvt_data_len);
	}

	bytes = write(soc_fw->out_fd, ec, (sizeof(*ec) + ec->pvt_data_len));
	if (bytes != (sizeof(*ec) + ec->pvt_data_len)) {
		fprintf(stderr, "error: can't write mixer %lu\n", (long unsigned int)bytes);
		return bytes;
	}
...
static int import_bytes_ext(struct soc_fw_priv *soc_fw,
	const struct snd_kcontrol_new *kcontrol)
{
	struct snd_soc_fw_bytes_ext *bc;
	struct soc_bytes_ext *bytes_ext =
		(struct soc_bytes_ext *)kcontrol->private_value;
	size_t bytes;

	bc = calloc(1, sizeof(*bc) + bytes_ext->pvt_data_len);
	if (!bc)
		return -ENOMEM;

	strncpy(bc->hdr.name, (const char *)kcontrol->name, SND_SOC_FW_TEXT_SIZE);
	bc->hdr.index = kcontrol->index |
		SOC_CONTROL_ID(kcontrol->get, kcontrol->put, 0);
	bc->hdr.access = kcontrol->access;
	bc->hdr.tlv_size = tlv_size(kcontrol);

	/* Fill private data */
	bc->max = bytes_ext->max;
	bc->pvt_data_len  = bytes_ext->pvt_data_len;
	if (bc->pvt_data_len > 0)
		memcpy(bc->pvt_data, bytes_ext->pvt_data, bytes_ext->pvt_data_len);

	verbose(soc_fw, "Bytes Ext: \"%s\" (g,p,i) %d:%d:%d\n",
		bc->hdr.name,
		SOC_CONTROL_GET_ID_GET(bc->hdr.index),
		SOC_CONTROL_GET_ID_PUT(bc->hdr.index),
		SOC_CONTROL_GET_ID_INFO(bc->hdr.index));

	bytes = write(soc_fw->out_fd, bc, sizeof(*bc) + bc->pvt_data_len);

	if (bytes != sizeof(*bc) + bc->pvt_data_len) {
		fprintf(stderr, "error: can't write mixer %lu\n", (long unsigned int)bytes);
		return bytes;
	}
...
static int import_bytes_tlv(struct soc_fw_priv *soc_fw,
	const struct snd_kcontrol_new *kcontrol)
{
	struct snd_soc_fw_bytes_ext *bc;
	struct soc_bytes_ext *bytes_ext =
	...
	bc->hdr.access = kcontrol->access;
	bc->hdr.tlv_size = tlv_size(kcontrol);
	bc->tlv_index = bytes_ext->tlv_index | SOC_CONTROL_ID_TLV(bytes_ext->get, bytes_ext->put);
	...
	verbose(soc_fw, "Bytes Tlv: \"%s\" index 0x%x tlv index 0x%x (g,p,i) %d:%d:%d\n",
		bc->hdr.name,
		bc->hdr.index,
		bc->tlv_index,
		SOC_CONTROL_GET_ID_GET(bc->hdr.index),
	...
	const struct snd_kcontrol_new *kcontrols, int kcontrol_count)
{
	...
	int i, mixers = 0, enums = 0, bytes_ext = 0;
	...
	for (i = 0; i < kcontrol_count; i++) {
		...
		case SOC_CONTROL_TYPE_BOOL_EXT:
			{
				struct soc_mixer_control *mixer =
					                        (struct soc_mixer_control *) kn->private_value;
				size += sizeof(struct snd_soc_fw_mixer_control);
				size += tlv_size(kn);
				size += mixer->pvt_data_len;
				mixers++;
			}
			break;
		...
		case SOC_CONTROL_TYPE_ENUM_EXT:
			{
			struct soc_enum *enum_control =
					(struct soc_enum *) kn->private_value;
			size += sizeof(struct snd_soc_fw_enum_control);
			size += tlv_size(kn);
			size += enum_control->pvt_data_len;
			enums++;
			}
			break;
		...
		case SOC_CONTROL_TYPE_BYTES_TLV:
			{
			struct soc_bytes_ext *bytes =
					(struct soc_bytes_ext *) kn->private_value;
			size += sizeof(struct snd_soc_fw_bytes_ext);
			size += bytes->pvt_data_len;
			bytes_ext++;
			}
			break;
	...
	for (i = 0; i < kcontrol_count; i++) {
		...
		case SOC_CONTROL_TYPE_ENUM_EXT:
			...
			break;
		case SOC_CONTROL_TYPE_BYTES_EXT:
			err = import_bytes_ext(soc_fw, kn);
			if (err < 0)
				return err;
			break;
		case SOC_CONTROL_TYPE_BYTES_TLV:
			err = import_bytes_tlv(soc_fw, kn);
			if (err < 0)
				return err;
			break;
		case SOC_CONTROL_TYPE_BYTES:
	...
	const struct snd_soc_fw_coeff *coeff)
{
	...
	verbose(soc_fw, " coeff 0x%x enum: \"%s\" R1/2 0x%x shift L/R %d/%d (g,p,i) %d:%d:%d\n",
	...
	int widget_count)
{
	...
	for (i = 0; i < widget_count; i++) {
		...
		if (widgets[i].pvt_data_len > 0)
			size += widgets[i].pvt_data_len;
	}
	...
	const struct snd_soc_dapm_widget *widgets, int widget_count)
{
	...
	for (i = 0; i < widget_count; i++) {
		...
		widget = calloc(1, sizeof(*widget) + widgets[i].pvt_data_len);
		if (!widget)
			return -ENOMEM;

		strncpy(widget->name, (const char *)widgets[i].name, SND_SOC_FW_TEXT_SIZE);
		...
		widget->event_type = widgets[i].event_type;
		widget->event_flags = widgets[i].event_flags;
		widget->pvt_data_len = widgets[i].pvt_data_len;

		verbose(soc_fw, " widget: \"%s\" R 0x%x shift %d pvt data len = %d\n",
			widget->name, widget->reg, widget->shift, widget->pvt_data_len);

		if (widgets[i].pvt_data_len > 0)
			memcpy(widget->pvt_data, widgets[i].priv, widgets[i].pvt_data_len);

		bytes = write(soc_fw->out_fd, widget, (sizeof(*widget) + widgets[i].pvt_data_len));
		if (bytes != (sizeof(*widget) + widgets[i].pvt_data_len)) {
