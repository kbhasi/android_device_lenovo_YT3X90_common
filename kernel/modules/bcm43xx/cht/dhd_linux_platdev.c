...
#define CUSTOM_WLAN_EN_GPIO_VALUE 303
...
static int wifi_plat_dev_drv_probe(struct platform_device *pdev)
{
	...
	err =  gpio_request(CUSTOM_WLAN_EN_GPIO_VALUE,"wlan-power");
	if (err < 0)
	{
	    printk(" request wlan-power gpio fail:%d  \n",err);
	}
	else
	{
	     printk(" request wlan-power gpio \n");
...
static int wifi_plat_dev_drv_probe_acpi(struct platform_device *pdev)
{
	...
	if (err < 0)
	{
	    printk("acpi request wlan-power gpio fail:%d  \n",err);
	}
	else
	{
	    printk("acpi request wlan-power gpio \n");
	...
	if (dev) {
		...
		if (acpi_bus_get_device(ACPI_HANDLE(dev), &adev) == 0) {
			...
			if (on) {
				...
				gpio_direction_output(CUSTOM_WLAN_EN_GPIO_VALUE ,1);
				OSL_SLEEP(120);
#else
				...
				gpio_direction_output(CUSTOM_WLAN_EN_GPIO_VALUE ,0);
#else
