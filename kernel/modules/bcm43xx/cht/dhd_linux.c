...
struct ether_addr default_mac_addr = {.octet={0x00, 0x90, 0x4c, 0x19, 0x80, 0x01}};
...
#endif /* CONFIG_WAKELOCK && LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27) */
...
#ifdef NAPI_SUPPORT
int dhd_napi_enable = 1;
module_param(dhd_napi_enable, int, 0644);
#endif /* NAPI_SUPPORT */
...
#ifdef NAPI_SUPPORT
static int dhd_not_to_be_called_napi_poll(struct napi_struct *napi, int budget)
{
	BCM_REFERENCE(napi);
	BCM_REFERENCE(budget);
	ASSERT(0);
	return 0;
}
#endif /* NAPI_SUPPORT */
...
#ifdef NAPI_SUPPORT
	netif_napi_add(ifp->net, &dhdinfo->napi, dhd_not_to_be_called_napi_poll, NAPI_POLL_WEIGHT);
#endif /* NAPI_SUPPORT */
