...
int wl_cfg80211_enforce_band(uint band)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	int err = 0;
	struct net_device *primary_ndev = NULL;
	...
	primary_ndev = bcmcfg_to_prmry_ndev(cfg);
	wldev_get_band(primary_ndev, &setband);
	if (setband == band) {
		/*Requesting same band, no need issue set band*/
		WL_ERR(("No need to set same band\n"));
		return BCME_ERROR;
	}
	if (cfg->p2p->vif_created) {
		WL_ERR(("Cannot set band when p2p is operational\n"));
		return BCME_ERROR;
	} else if (wl_get_drv_status(cfg, CONNECTED, primary_ndev)) {
		WL_ERR(("wlan0: disconnecting %s\n", primary_ndev->name));
		err = wl_cfg80211_disconnect(bcmcfg_to_wiphy(cfg),
					primary_ndev, UNSPECIFIED_REASON);
		if (err) {
			printf("wlan0: Unable to disconnect wlan0\n");
			return err;
		}
		cfg80211_disconnected(primary_ndev, 0, NULL, 0, true, GFP_KERNEL);
	}
	return err;
}
