...
dhdpcie_pme_active(osl_t *osh, bool enable)
{
	...
	/* Not setting PME_CSR_PME_STAT when enable is true is a hack to
	 * trigger a PME irq right away while suspending in case it 'got' set
	 * after unsetting PME_EN, which can 'understandably', though rarely,
	 * occur after toggling it if we get woken by something else.
	 * In that case here, we workaround it by avoiding to ack the PME.
	 * This looks like a d3_delay issue, but even if we are out of specs
	 * with Linux's 10ms (the 4356 advertizes at least (Trefclkstable
	 * + Tref2perst + Tperst2firstconfig = 10ms + 100us + 6ms) when woken by
	 * the 4356 and (probably way) more when not), even a 200ms one doesn't
	 * succeed in getting rid of it, so maybe a FW or power seq. issue (
	 * maybe we need to do something special when not woken by the 4356, the
	 * DSDT isn't clear to me) ?
	 * Also, pm_wakeup_event and such seem to require a rework, pme_poll
	 * doesn't fit since it gets disabled upon first unpolled PME. */
	if (enable) {
