...
struct pci_bus                  *unc_pkg_to_bus_map;
U32                              pkg = 0;
...
)
{
        ...
        for (device_no = 0; device_no < MAX_PCI_DEVNO; device_no++) {
            for (function_no = 0; function_no < MAX_PCI_FUNCNO; function_no++) {
                ...
                if (num_packages <= 8 ) {   
                    ...
                    for (i = 0; i < 8; i++, mapping = mapping >> 3) {
                    ...
                    // for sockets > 8 machines
                    if (bus_no != busno_prev) {
                        unc_package_to_bus_map[pkg_num] = (U32)bus_no;
                        busno_prev = bus_no;
                        pkg_num++;
                    }
                }
    ...
    for (i = 0; i< num_packages; i++) {
         SEP_PRINT_DEBUG("package[%d] is on bus [%d]\n", i, unc_package_to_bus_map[i]);
    }
    return OS_SUCCESS;
}
...
)
{
    ...
    if (UNC_COMMON_Do_Bus_to_Socket_Map(ubox_did) != OS_SUCCESS) {
        return;
    }
...
)
{
    ...
    if (unc_pkg_to_bus_map) {
        unc_pkg_to_bus_map = CONTROL_Free_Memory(unc_pkg_to_bus_map);
    }
...
/************************************************************/
/* 
 * UNC common PCI  Linux based API
 *
 ************************************************************/

extern VOID
UNC_COMMON_PCI_Lin_Write_PMU (
    PVOID            param,
    U32              ubox_did,
    U32              control_msr,
    U32              ctl_val,
...
)
{
    ...
    FOR_EACH_REG_ENTRY_UNC(pecb,dev_idx,idx) {
        ...
        devfn = (ECB_entries_dev_no(pecb,idx) << 3) | (ECB_entries_func_no(pecb,idx) & 0x7);
        ...
        if (ctl_val                                  &&
            ...
            (ECB_entries_reg_type(pecb,idx) == CCCR) &&
             callback->is_Unit_Ctl(ECB_entries_reg_id(pecb,idx))) {
             value = ctl_val;
             pci_bus_write_config_dword(&unc_pkg_to_bus_map[package_num], devfn, ECB_entries_reg_id(pecb,idx), value);
             SEP_PRINT_DEBUG("UNC_COMMON_PCI_Write_PMU cpu=%d, reg = 0x%x --- value 0x%x\n",
                             this_cpu, ECB_entries_reg_id(pecb,idx), value);
             // Manually unset the reset bit to resolve sticky bit issue in KNL
             pci_bus_write_config_dword(&unc_pkg_to_bus_map[package_num], devfn, ECB_entries_reg_id(pecb,idx), 0);
             SEP_PRINT_DEBUG("UNC_COMMON_PCI_Write_PMU cpu=%d, reg = 0x%x --- value 0x%x\n",
                             this_cpu, ECB_entries_reg_id(pecb,idx), value);
             continue;
        }

        pci_bus_write_config_dword(&unc_pkg_to_bus_map[package_num], devfn, ECB_entries_reg_id(pecb,idx), (U32)ECB_entries_reg_value(pecb,idx));
        SEP_PRINT_DEBUG("UNC_COMMON_PCI_Write_PMU cpu=%d, reg = 0x%x --- value 0x%x\n",
                             this_cpu, ECB_entries_reg_id(pecb,idx), (U32)ECB_entries_reg_value(pecb,idx));
        ...
        // we need to zero out the upper bits as well
        if (ECB_entries_reg_type(pecb,idx) == DATA) {
            pci_bus_write_config_dword(&unc_pkg_to_bus_map[package_num], devfn, (ECB_entries_reg_id(pecb,idx) + NEXT_ADDR_OFFSET), (U32)ECB_entries_reg_value(pecb,idx));            
            SEP_PRINT_DEBUG("UNC_COMMON_PCI_Write_PMU cpu=%d, reg = 0x%x --- value 0x%x\n",
                             this_cpu, ECB_entries_reg_id(pecb,idx), (U32)ECB_entries_reg_value(pecb,idx));
        }
...
)
{
    ...
    FOR_EACH_REG_ENTRY_UNC(pecb, dev_idx, i) {
        ...
        devfn = (ECB_entries_dev_no(pecb,i) << 3) | (ECB_entries_func_no(pecb,i) & 0x7);
        if (callback                               &&
            callback->is_PMON_Ctl                  &&
            (ECB_entries_reg_type(pecb,i) == CCCR) &&
             callback->is_PMON_Ctl(ECB_entries_reg_id(pecb,i))) {
             value = enable_val | ECB_entries_reg_value(pecb,i);
             pci_bus_write_config_dword(&unc_pkg_to_bus_map[package_num], devfn, ECB_entries_reg_id(pecb,i), value);
             SEP_PRINT_DEBUG("UNC_COMMON_PCI_Enable_PMU Event_reg = 0x%x --- value 0x%x\n",
                          ECB_entries_reg_id(pecb,i), value);
             continue;
        }
        if (disable_val                            &&
            ...
            callback->is_Unit_Ctl                  &&
            callback->is_Unit_Ctl(ECB_entries_reg_id(pecb,i))) {
            pci_bus_read_config_dword(&unc_pkg_to_bus_map[package_num], devfn, ECB_entries_reg_id(pecb,i), (U32 *)&value);
            value &= ~(disable_val);
            pci_bus_write_config_dword(&unc_pkg_to_bus_map[package_num], devfn, ECB_entries_reg_id(pecb,i), value);
            SEP_PRINT_DEBUG("UNC_COMMON_PCI_Enable_PMU Event_reg = 0x%x --- value 0x%x\n",
                         ECB_entries_reg_id(pecb,i), value);
...
extern VOID
UNC_COMMON_PCI_Lin_Disable_PMU (
    PVOID               param,
    U32                 control_msr,
    U32                 enable_val,
    U32                 disable_val,
...
extern  VOID
UNC_COMMON_PCI_Lin_Read_Counts (
    PVOID  param,
    U32    id
)
{
    ...
    *data   = cur_grp + 1;
    ...
    FOR_EACH_DATA_REG_UNC(pecb, id, i) {
        ...
        devfn = (ECB_entries_dev_no(pecb,i) << 3) | (ECB_entries_func_no(pecb,i) & 0x7);
        // read lower 4 bytes
        pci_bus_read_config_dword(&unc_pkg_to_bus_map[package_num], devfn, ECB_entries_reg_id(pecb,i), (U32 *)&value_low);
        value_low &= LOWER_4_BYTES_MASK;

        // read upper 4 bytes
        pci_bus_read_config_dword(&unc_pkg_to_bus_map[package_num], devfn, (ECB_entries_reg_id(pecb,i) + NEXT_ADDR_OFFSET), (U32 *)&value_high);
        *data = (value_high << NEXT_ADDR_SHIFT) | value_low;
    } END_FOR_EACH_DATA_REG_UNC;
...
extern VOID
UNC_COMMON_PCI_Lin_Read_PMU_Data(
    PVOID           param
)
{
    U32             devfn;
...
extern VOID
UNC_COMMON_PCI_Lin_Scan_For_Uncore(
    PVOID           param,
    U32             dev_node,
    CALLBACK_LIN    callback
)
{
    U32                        i, j, k;
    struct pci_dev            *pdev;
    ...
    if (unc_pkg_to_bus_map == NULL) {
        unc_pkg_to_bus_map = CONTROL_Allocate_Memory(num_packages * sizeof(struct pci_bus));
        if (unc_pkg_to_bus_map == NULL) {
            SEP_PRINT_DEBUG("UNC_COMMON_PCI_Lin_Scan_For_Uncore allocated NULL by CONTROL_Allocate_Memory\n");
            return;
        }
    }

    if ( callback && callback->init_Device_IDs) {
        callback->init_Device_IDs();
    }
    if ( callback && callback->get_Device_IDs) {
        device_ids = callback->get_Device_IDs(&num_devs);
        if (device_ids == NULL) {
            return;
        }
    }
    for (i = 0; i < num_devs; i++) {
        pdev = pci_get_device(PCI_VENDOR_ID_INTEL, device_ids[i], NULL);
        if (pdev == NULL) {
            continue;
        }
        while (pdev != NULL) {
            for (j=0; j< MAX_PCI_DEVNO;j++) {
                ...
                for(k=0;k<MAX_PCI_FUNCNO;k++) {
                    ...
                    if ((j == ((pdev->devfn >> 3) & 0x1F)) && (k == (pdev->devfn & 0x7))) {
                        UNCORE_TOPOLOGY_INFO_pcidev_is_found_in_platform(&uncore_topology, dev_node, j, k) = 1;
                        SEP_PRINT("found device 0x%x at B:D:F = %d:%d:%d\n", device_ids[i],pdev->bus->number,j,k);
                        if ((pdev->bus->number != busno_prev) && (pkg < num_packages)) {
                            unc_pkg_to_bus_map[pkg] = pdev->bus[0];
                            pkg++;
                            busno_prev = pdev->bus->number;
                            SEP_PRINT("Updating bus number of package %d to %d, busno is at %d\n",pkg-1,pdev->bus->number,busno_prev);
                        }
                    }
                }
            }
            pdev = pci_get_device(PCI_VENDOR_ID_INTEL, device_ids[i], pdev);
        }
    }    
