...
#if !(defined(CONFIG_PREEMPT_RT) || defined(CONFIG_PREEMPT_RT_FULL) || defined(DRV_USE_NMI))
            else {
        ...
        for (i=0; i<OUTPUT_NUM_BUFFERS; i++) { //iterate through all buffers
            ...
            if ((to_copy = OUTPUT_buffer_full(outbuf, cur_buf))) {
                if (flush && DRV_CONFIG_enable_cp_mode(pcfg) && cur_buf == OUTPUT_current_buffer(outbuf)) {
                    ...
                    OUTPUT_remaining_buffer_size(outbuf) = OUTPUT_total_buffer_size(outbuf)-OUTPUT_buffer_full(outbuf,OUTPUT_current_buffer(outbuf));
                }
                break;
            }
        }
