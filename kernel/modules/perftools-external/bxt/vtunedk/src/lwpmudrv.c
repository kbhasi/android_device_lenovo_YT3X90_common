...
)
{
    ...
    for (i = 0; i < num_devices; i++) {
        ...
        LWPMU_DEVICE_to_read_data(&devices[i]) = CONTROL_Allocate_Memory(num_packages * sizeof(U64 *));
        LWPMU_DEVICE_current_data(&devices[i]) = CONTROL_Allocate_Memory(num_packages * sizeof(U64 *));
        LWPMU_DEVICE_prev_value(&devices[i]) = CONTROL_Allocate_Memory(num_packages * sizeof(U64 *));
        for (j = 0; j < num_packages; j++) {
            // Allocate memory and zero out base value array (one per group)
            ...
            // Allocate memory and zero out to_read_data, current_data and prev_value arrays (one across groups)
            LWPMU_DEVICE_to_read_data(&devices[i])[j] = CONTROL_Allocate_Memory((max_entries+1) * sizeof(U64));
            LWPMU_DEVICE_current_data(&devices[i])[j] = CONTROL_Allocate_Memory((max_entries+1) * sizeof(U64));
            LWPMU_DEVICE_prev_value(&devices[i])[j] = CONTROL_Allocate_Memory(max_entries * sizeof(U64));
            LWPMU_DEVICE_to_read_data(&devices[i])[j][0] = 0LL;   //Group ID is 0 initially
            LWPMU_DEVICE_current_data(&devices[i])[j][0] = 0LL;       
            for (k = 1; k <= max_entries; k++) {
                LWPMU_DEVICE_to_read_data(&devices[i])[j][k] = 0LL;
                LWPMU_DEVICE_current_data(&devices[i])[j][k] = 0LL;
                LWPMU_DEVICE_prev_value(&devices[i])[j][k-1] = 0LL;
            }
        }
        max_entries = 0;

        ecb = LWPMU_DEVICE_PMU_register_data(&devices[i])[0];
        LWPMU_DEVICE_acc_per_thread(&devices[i]) = CONTROL_Allocate_Memory(GLOBAL_STATE_num_cpus(driver_state) *
                                                                                sizeof(VOID *));
        LWPMU_DEVICE_prev_val_per_thread(&devices[i]) = CONTROL_Allocate_Memory(GLOBAL_STATE_num_cpus(driver_state) *
                                                                                     sizeof(VOID *));
        for (j = 0; j < GLOBAL_STATE_num_cpus(driver_state); j++) {
             LWPMU_DEVICE_acc_per_thread(&devices[i])[j]      = CONTROL_Allocate_Memory(ECB_num_events(ecb) *
                                                                LWPMU_DEVICE_num_units(&devices[i]) * sizeof(U64));
             LWPMU_DEVICE_prev_val_per_thread(&devices[i])[j] = CONTROL_Allocate_Memory(ECB_num_events(ecb) * 
                                                                LWPMU_DEVICE_num_units(&devices[i]) * sizeof(U64));
             // initialize all values to 0
             for (k = 0; k < (ECB_num_events(ecb)*LWPMU_DEVICE_num_units(&devices[i])); k++) {
                  LWPMU_DEVICE_acc_per_thread(&devices[i])[j][k]      = 0LL;
                  LWPMU_DEVICE_prev_val_per_thread(&devices[i])[j][k] = 0LL;
             }
        }
    }
...
)
{
    ...
    if (LWPMU_DEVICE_to_read_data(&devices[i])) {
        for (j = 0; j < num_packages; j++) {
            LWPMU_DEVICE_to_read_data(&devices[i])[j] = CONTROL_Free_Memory(LWPMU_DEVICE_to_read_data(&devices[i])[j]);
        }
        LWPMU_DEVICE_to_read_data(&devices[i]) = CONTROL_Free_Memory(LWPMU_DEVICE_to_read_data(&devices[i]));
    }
    if (LWPMU_DEVICE_current_data(&devices[i])) {
        for (j = 0; j < num_packages; j++) {
            LWPMU_DEVICE_current_data(&devices[i])[j] = CONTROL_Free_Memory(LWPMU_DEVICE_current_data(&devices[i])[j]);
        }
        LWPMU_DEVICE_current_data(&devices[i]) = CONTROL_Free_Memory(LWPMU_DEVICE_current_data(&devices[i]));
    }
    ...
    if (LWPMU_DEVICE_acc_per_thread(&devices[i])) {
        for (j = 0; j < GLOBAL_STATE_num_cpus(driver_state); j++) {
            LWPMU_DEVICE_acc_per_thread(&devices[i])[j] = CONTROL_Free_Memory(LWPMU_DEVICE_acc_per_thread(&devices[i])[j]);
        }
        LWPMU_DEVICE_acc_per_thread(&devices[i]) = CONTROL_Free_Memory(LWPMU_DEVICE_acc_per_thread(&devices[i]));
    }
    if (LWPMU_DEVICE_prev_val_per_thread(&devices[i])) {
        for (j = 0; j < GLOBAL_STATE_num_cpus(driver_state); j++) {
             LWPMU_DEVICE_prev_val_per_thread(&devices[i])[j] = CONTROL_Free_Memory(LWPMU_DEVICE_prev_val_per_thread(&devices[i])[j]);
        }
        LWPMU_DEVICE_prev_val_per_thread(&devices[i]) = CONTROL_Free_Memory(LWPMU_DEVICE_prev_val_per_thread(&devices[i]));
    }
    ...
    if (max_groups_unc > 1) {
        ...
        for(i = 0; i < num_devices; i++) {
            ...
            if (LWPMU_DEVICE_em_groups_count(&devices[i]) > 1) {
                ...
                LWPMU_DEVICE_cur_group(&devices[i])++;
                LWPMU_DEVICE_cur_group(&devices[i]) %= LWPMU_DEVICE_em_groups_count(&devices[i]);
                // Post group switch - Update to_read_data with new group data
                ...
                for (j = 0; j < num_packages; j++) {
                    LWPMU_DEVICE_current_data(&devices[i])[j][0] = cur_grp;
                    for (k = 1; k <= (ECB_num_events(ecb_unc)*num_units); k++) {
                        LWPMU_DEVICE_current_data(&devices[i])[j][k] = LWPMU_DEVICE_acc_value(&devices[i])[j][cur_grp][k-1];
                        LWPMU_DEVICE_prev_value(&devices[i])[j][k-1] = 0LL;  //zero out prev_value for new collection
                    }
                    temp = LWPMU_DEVICE_to_read_data(&devices[i])[j];
                    LWPMU_DEVICE_to_read_data(&devices[i])[j] = LWPMU_DEVICE_current_data(&devices[i])[j];
                    LWPMU_DEVICE_current_data(&devices[i])[j] = temp;
                }
            }
        }
...
)
{
    ...
    if (xen_initial_domain()) {
        SEP_PRINT_ERROR("lwpmu driver cannot be loaded on XEN dom0!\n");
        return status;
    }
#endif
