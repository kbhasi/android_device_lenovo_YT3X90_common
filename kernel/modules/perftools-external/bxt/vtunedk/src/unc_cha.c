...
/***********************************************************************
 *
 * dispatch function for SKX CHA
 *
 ***********************************************************************/

static DRV_BOOL
unc_cha_skx_is_Unit_Ctl (
    U32 msr_addr
)
{
    return (IS_THIS_SKX_BOX_CTL_MSR(msr_addr));
}

static DRV_BOOL
unc_cha_skx_is_PMON_Ctl (
    U32 msr_addr
)
{
    return (IS_THIS_SKX_EVSEL_PMON_CTL_MSR(msr_addr));
}

DEVICE_CALLBACK_NODE  unc_cha_skx_callback = {
    NULL,
    NULL,
    unc_cha_skx_is_Unit_Ctl,
    unc_cha_skx_is_PMON_Ctl
...
/******************************************************************************************
 * @fn          static VOID unc_cha_skx_Write_PMU(VOID*)
 *
 * @brief       Initial write of PMU registers
 *              Walk through the enties and write the value of the register accordingly.
 *
...
/******************************************************************************************
 * @fn         static VOID unc_cha_skx_Disable_PMU(PVOID)
 *
 * @brief      Disable the per unit global control to stop the PMU counters.
 *
 * @param      Device Index of this PMU unit
 ...
 ******************************************************************************************/
static VOID
unc_cha_skx_Disable_PMU (
    PVOID  param
)
{
    UNC_COMMON_MSR_Disable_PMU(param,
                               SKX_UBOX_GLOBAL_CONTROL_MSR,
                               DISABLE_SKX_CHA_COUNTERS,
                               0,
                               &unc_cha_skx_callback);

    return;
}

/******************************************************************************************
 * @fn         static VOID unc_cha_skx_Enable_PMU(PVOID)
 *
 * @brief      Set the enable bit for all the EVSEL registers
 *
 * @param      Device Index of this PMU unit
 ...
 * Initialize the dispatch table
 */
DISPATCH_NODE  skx_cha_dispatch =
{
    NULL,                                // initialize
    NULL,                                // destroy
    unc_cha_skx_Write_PMU,               // write
    unc_cha_skx_Disable_PMU,             // freeze
    unc_cha_skx_Enable_PMU,              // restart
    UNC_COMMON_MSR_Read_PMU_Data,        // read
    NULL,                                // check for overflow
    NULL,                                //swap group
    NULL,                                //read lbrs
