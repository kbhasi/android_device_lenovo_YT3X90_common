...
/* ------------------------------------------------------------------------- */
/*!
 * @fn extern int PCI_Read(bus, dev_no, func_no, reg_id)
 *
 * @param   bus - PCI bus info struct
 * @param   dev_no - PCI device number
 * @param   func_no - PCI function number
 * @param   reg_id - PCI register offset
 *
 * @return  value at this location
 *
 * @brief   Reads a ULONG from PCI extended configuration space
 *
 */
extern U32
PCI_Read (
...
)
{
    ...
    if (reg_id < 0xff) {
        pci_address = FORM_PCI_ADDR(bus->number, dev_no, func_no, reg_id);
        value = PCI_Read_Ulong(pci_address);
    }
    else {
        devfn = (dev_no << 3) | (func_no & 0x7);
        bus->ops->read(bus, devfn, reg_id, 4, &value);
    }
...
/* ------------------------------------------------------------------------- */
/*!
 * @fn extern int PCI_Write(bus, dev_no, func_no, reg_id)
 *
 * @param   bus - PCI bus info struct
 * @param   dev_no - PCI device number
 * @param   func_no - PCI function number
 ...
 * @return  VOID
 *
 * @brief   Writes a ULONG to PCI extended configuration space
 *
 */
extern void
PCI_Write (
...
)
{
    ...
    if (reg_id < 0xff) {
        ...
        devfn = (dev_no << 3) | (func_no & 0x7);
        bus->ops->write(bus, devfn, reg_id, 4, value);
    }
