...
{
	printk("CAMERA: start check ov8865 gpio\n");
...
static int ov8865_platform_init(struct i2c_client *client)
{
	int ret;
	...
	if (ret) {
		pr_err("platform ov8865 %s: failed to request PWDN gpio(pin %d) keep going\n",
	...
	if (ret) {
		pr_err("platform ov8865 %s: failed to request RST gpio(pin %d) keep going\n",
...
static int ov8865_platform_deinit(void)
{
	printk("%s %d\r\n", __func__, __LINE__);
...
static struct camera_sensor_platform_data ov8865_sensor_platform_data = {
	...
	.platform_init = ov8865_platform_init,
	.platform_deinit = ov8865_platform_deinit,
};
...
EXPORT_SYMBOL_GPL(ov8865_platform_data);
