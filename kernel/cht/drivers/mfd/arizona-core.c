...
//#define UART_GPIO
// gpio294 is used to witch uart/audiojack path
// gpio294 is requested by drivers/misc/uart_audiojack_sw.c
// so do not request gpio294 in this driver
...
static int arizona_runtime_resume(struct device *dev)
{
    ...
    if(!arizona->dcvdd_state) {
        ...
        msleep(5);   /*delay 5s for power up ready*/ 
	...
	dev_dbg(arizona->dev, "Leaving AoD mode End\n");
...
int arizona_dev_init(struct arizona *arizona)
{
...
#ifdef UART_GPIO
	ret = gpio_request_one(294,GPIOF_DIR_OUT | GPIOF_INIT_LOW,
				       "arizona / uart_switch");
		dev_err(dev, "request 294/uart swtich: %d\n", ret);	
#endif
	...
	default:
        ...
        pr_info("in %s arizona->pdata.irq_flags=%d,arizona->pdata.reset=%d\n", __func__,arizona->pdata.irq_flags,arizona->pdata.reset);
	if (arizona->pdata.reset) {
		gpio_set_value_cansleep(arizona->pdata.reset, 1);
		msleep(1);
	}
	...
	pr_info("in %s arizona->pdata.irq_flags=%d\n", __func__,arizona->pdata.irq_flags);
