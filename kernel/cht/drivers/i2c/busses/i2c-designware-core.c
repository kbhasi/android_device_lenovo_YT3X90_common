...
/* support DLP for BladeIII */
#if defined (CONFIG_MIPI_DLP) || defined (CONFIG_HDMI_DLP)
#define DLP_I2C_BUS_NUM     4		/* 5-1, start from 0. */
#endif
...
#if defined (CONFIG_MIPI_DLP) || defined (CONFIG_HDMI_DLP)
	if (dev->adapter.nr == DLP_I2C_BUS_NUM) {
		dev_dbg(dev->dev, "[dlp]--1-- SDA delay = %d.\n", dw_readl(dev, 0x7c));
		dw_writel(dev, 0x80, 0x7c);
		dev_dbg(dev->dev, "[dlp]--2-- SDA delay = %d.\n", dw_readl(dev, 0x7c));
		dev_dbg(dev->dev, "[dlp] dev->adapter.nr = %d, dev->master_cfg = 0x%x.\n", dev->adapter.nr, dev->master_cfg);
		dev->master_cfg = (dev->master_cfg & ~DW_IC_CON_SPEED_FAST) | DW_IC_CON_SPEED_STD;
		dev_dbg(dev->dev, "[dlp] hardcode to set bus speed to standard. dev->master_cfg = 0x%x.\n", dev->master_cfg);
	}
#endif
...
//liulc1  add 200ns
       if ((dev->adapter.nr ==0) || (dev->adapter.nr == 1)) {
               dw_writel(dev, 0x14, 0x7c);
      }
//liulc1  end
