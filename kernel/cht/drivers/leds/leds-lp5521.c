...
static int lp5521_suspend(struct i2c_client *client)
{
	u8 reg;
	struct lp55xx_led *led = i2c_get_clientdata(client);
	struct lp55xx_chip *chip = led->chip;
	enum lp55xx_engine_mode mode = chip->engines[0].mode;

	if (mode == LP55XX_ENGINE_DISABLED) {
		lp55xx_update_bits(chip, LP5521_REG_ENABLE, LP5521_MASTER_ENABLE, 0);

		//if (gpio_is_valid(chip->pdata->enable_gpio)) {
		//	gpio_set_value(chip->pdata->enable_gpio, 0);
		//}

		dev_info(&client->dev, "lp5521 suspend\n");
	}

	lp55xx_read(chip, LP5521_REG_ENABLE, &reg);
	dev_info(&client->dev, "lp5521 suspend: LP5521_REG_ENABLE = 0x%2x\n", reg);
...
static int lp5521_resume(struct i2c_client *client)
{
	...
	lp55xx_update_bits(chip, LP5521_REG_ENABLE, LP5521_MASTER_ENABLE, LP5521_MASTER_ENABLE);
	...
	lp55xx_read(chip, LP5521_REG_ENABLE, &reg);
	dev_info(&client->dev, "lp5521 resume: LP5521_REG_ENABLE = 0x%2x\n", reg);
