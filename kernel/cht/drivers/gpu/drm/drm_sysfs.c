...
#if defined (CONFIG_MIPI_DLP) || defined (CONFIG_HDMI_DLP)
void drm_sysfs_power_key_up(void)
{
	extern struct drm_device *gdev;
       struct drm_device *dev  = gdev;
       static unsigned long last_power_event;

	char *event_string = "POWERKEYUP=1";
	char *envp[] = { event_string, NULL };
       if (jiffies_to_msecs(jiffies) - last_power_event < 1000)
		return;
	DRM_DEBUG("generating powerkey up\n");

	kobject_uevent_env(&dev->primary->kdev->kobj, KOBJ_CHANGE, envp);
       last_power_event = jiffies_to_msecs(jiffies);
}
#endif
