...
/*In testmode, disabled Jackdetection ISR and set HEADSET_SW to high (make sure codec in work mode before set)*/
static ssize_t arizona_extcon_testmode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (err)
		return err;
	info->test_mode = (val != 0);
	pm_runtime_get_sync(info->arizona->dev);
	mutex_lock(&info->arizona->reg_setting_lock);
	if (info->test_mode){
		regmap_update_bits(info->arizona->regmap, ARIZONA_JACK_DETECT_ANALOGUE, ARIZONA_JD1_ENA_MASK, 0);
		regmap_write(info->arizona->regmap, ARIZONA_GPIO3_CTRL, 0x2901);
		dev_dbg(info->arizona->dev, "arizona_extcon_testmode_store(), enter test mode:%d\n", info->test_mode);
        }
	else{
		regmap_update_bits(info->arizona->regmap, ARIZONA_JACK_DETECT_ANALOGUE, ARIZONA_JD1_ENA_MASK, ARIZONA_JD1_ENA);
		regmap_write(info->arizona->regmap, ARIZONA_GPIO3_CTRL, 0xA101); 
		pm_runtime_put_autosuspend(info->arizona->dev);	
		dev_dbg(info->arizona->dev, "arizona_extcon_testmode_store(), quit test mode:%d\n", info->test_mode);
        }
	mutex_unlock(&info->arizona->reg_setting_lock);
	return size;
}

static ssize_t arizona_extcon_testmode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);	
	struct arizona_extcon_info *info = platform_get_drvdata(pdev);
...
DEVICE_ATTR(test_mode, 0644, arizona_extcon_testmode_show, arizona_extcon_testmode_store);
...
static const char *arizona_extcon_get_micbias(struct arizona_extcon_info *info)
{
	...
	dev_dbg(arizona->dev, "arizona_extcon_get_micbias SetrightBias =%d\n", SetrightBias);
	switch (SetrightBias) { 	
	case 1:
		return "MICBIAS1";
	case 2:
...
int arizona_micd_mic_reading(struct arizona_extcon_info *info, int val)
{
	...
	if (val > info->micd_ranges[0].max &&
		...
		if (info->jack_flips >= info->micd_num_modes * 10) {
			...
			info->jack_flips++;
			dev_dbg(arizona->dev, "flips = %d, micd_mode = %d, micd_num =%d\n", info->jack_flips, info->micd_mode, info->micd_num_modes);
			return -EAGAIN;
		}
	}
...
static int arizona_extcon_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(&pdev->dev, &dev_attr_test_mode);
	if (ret != 0)
		dev_err(&pdev->dev,
			"Failed to create sysfs node for test_mode %d\n",
			ret);
	return 0;
