...
void dump_pmic_tt_regs(void)
{
	...
	dev_err(chc.dev, "PMIC CHRGR TT dump\n");
	dev_err(chc.dev, "====================\n");
...
static int get_charger_type(void)
{
	...
	do {
		...
		if (ret)
			return 0;
		i++;
		dev_dbg(chc.dev, "pmic==== Read USBSRCDETSTATUS val: %x\n", val);
...
for(h=0;h<2;h++)
{
       ...
       if(chgr_type == PMIC_CHARGER_TYPE_SDP) {
               ...
               do {
                       ...
                       if (ret)
                               return 0;
                       i++;
                       dev_err(chc.dev, "pmic===Read again USBSRCDETSTATUS val: %x\n", val);
               ...
               if ((val & USBSRCDET_SUSBHWDET_DETSUCC) != USBSRCDET_SUSBHWDET_DETSUCC) {   /* using firstly chgr_type */
                       dev_err(chc.dev, "pmic===Charger detection again unsuccessful after %dms\n", i * USBSRCDET_SLEEP_RETRYDET);
               } else {
                       chgr_type = (val & USBSRCDET_USBSRCRSLT_MASK) >> 2;
                       dev_dbg(chc.dev, "pmic===Charger type after detection again complete: %d\n", (val & USBSRCDET_USBSRCRSLT_MASK) >> 2);
               }
       }
}
//liulc1  end
...
#define ECOMPASS_RST_N		(gpio_east_base+21)
#define ACC_M_INT2		(gpio_east_base+18)
#define ACC_M_INT		(gpio_east_base+15)
#define ALS_INT_N		(gpio_east_base+25)
...
static int pmic_chrgr_shutdown(struct platform_device *pdev)
{
	...
	ret = gpio_request(ECOMPASS_RST_N, "ECOMPASS_RST_N");
	if(ret) {
		printk("gpio_request ECOMPASS_RST_N error\n");
	}
	gpio_direction_output(ECOMPASS_RST_N, 0);

	ret = gpio_request(ACC_M_INT2, "ACC_M_INT2");
	if(ret) {
		printk("gpio_request ACC_M_INT2 error\n");
	}
	gpio_direction_output(ACC_M_INT2, 0);
	...
	//disable  TP power	
	intel_soc_pmic_writeb(0x6E9B,0x00);
