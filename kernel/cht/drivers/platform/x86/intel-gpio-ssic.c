				...
				struct usb_device *udev)
{
	...
	if (!handle) {
		printk(KERN_ERR "No acpi handle found!\n");
		return false;
	}

	if (acpi_bus_get_device(handle, &device)) {
		printk(KERN_ERR "Function acpi_bus_get_device failed!\n");
		return false;
	}
...
static irqreturn_t gpio_ssic_wakeup_irq(int irq, void *__ssic)
{
	...
	/* Take 3s wake lock */
	wake_lock_timeout(&ssic->wake_lock, msecs_to_jiffies(3000));

	usb_lock_device(udev);
	printk(KERN_ERR "%s (under_lock) before resume\n", __func__);
	if (!usb_autoresume_device(udev))
		usb_autosuspend_device(udev);
	printk(KERN_ERR "%s (under_lock) after resume\n", __func__);
...
static int gpio_ssic_request_irq(struct gpio_ssic *ssic)
{
	...
	if (ret) {
		dev_err(dev, "failed to request IRQ for irq_wakeup (%d)\n", ret);
		return ret;
	}
				...
				struct usb_device *udev)
{
	...
	if (is_ssic_roothub(ssic, udev)) {
		...
		dev_dbg(ssic->dev, "%s: SSIC roothub added\n", __func__);
		...
		dev_dbg(ssic->dev, "%s: SSIC device added\n", __func__);
				...
				struct usb_device *udev)
{
	...
	if (is_ssic_roothub(ssic, udev)) {
		...
		dev_dbg(ssic->dev, "%s: SSIC roothub removed\n", __func__);
...
static int gpio_ssic_init_gpios(struct gpio_ssic *ssic)
{
	...
	if (ret) {
		dev_err(dev, "Can't configure gpio_wakeup as input (%d)\n", ret);
		return ret;
	}

	ret = gpiod_to_irq(ssic->gpio_wakeup);
	if (ret < 0) {
		dev_err(dev, "can't get valid irq num for gpio wakeup (%d)\n", ret);
		return ret;
	}
	ssic->irq_wakeup = ret;
	...
	wake_lock_init(&ssic->wake_lock, WAKE_LOCK_SUSPEND, "gpio-ssic-wakelock");
...
static void gpio_ssic_wakeup_connect(struct seq_file *s, void *unused)
{
	struct gpio_ssic	*ssic = s->private;
	gpio_ssic_wakeup_irq(0, ssic);
}

static void gpio_ssic_interrupt_open(struct inode *inode, struct file *file)
{
...
static int gpio_ssic_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "Intel SSIC driver probe succeed\n");
...
static int gpio_ssic_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&ssic->wake_lock);
