...
static ssize_t hideep_reset_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct ist510e *ts = i2c_get_clientdata(client);

    //for hideep developer use it in user image.
    ISTCORE_INFO("reset by command reading\n");
    istcore_lcd_on_reset_ic();
    return sprintf(buf, "reset by command!\n");
}

static ssize_t hideep_reset_store(struct device *dev,
                         struct device_attribute *attr,
                         const char *buf, size_t count)
{
    struct i2c_client *client = to_i2c_client(dev);
...
static ssize_t anypen_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return scnprintf(buf, PAGE_SIZE,"anypen_switch = %d\n", anypen_switch);
}
void anypen_switch_operate(void)
{
	u8 cmd = 0;
	...
	if(anypen_switch==1)
		{
	cmd=0x01;
	istcore_i2c_write(ts, IST_ANYPEN_SWITCH, 1, &cmd);
	ISTCORE_INFO("anypen open 0x%x .\n",cmd);
		}
	else
		{
	cmd=0x00;
	istcore_i2c_write(ts, IST_ANYPEN_SWITCH, 1, &cmd);
	ISTCORE_INFO("anypen off 0x%x .\n",cmd);

		}
}
static ssize_t anypen_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int temp;
    struct i2c_client *client = to_i2c_client(dev);
    struct ist510e *ts = i2c_get_clientdata(client);
    ...
    anypen_switch = temp;
    ISTCORE_INFO("anypen_switch_store anypen_switch %d .\n",anypen_switch);
	anypen_switch_operate();
    return count;
}
...
#endif
static DEVICE_ATTR(anypen_switch, S_IRWXUGO,anypen_switch_show,anypen_switch_store);
