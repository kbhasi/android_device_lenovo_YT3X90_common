...
extern void touch_set_pmic_power(bool flag);
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data,
		bool rebuild);
static void synaptics_rmi4_anypen_switch_op(struct synaptics_rmi4_data *rmi4_data);
#ifdef CONFIG_FB
...
static int synaptics_power_hal_suspend_init(struct device *dev);
static void synaptics_power_hal_suspend_destroy(struct device *dev);

static void synaptics_power_hal_suspend(struct device *dev);
static void synaptics_power_hal_resume(struct device *dev);
#endif
...
static ssize_t synaptics_rmi4_anypen_switch_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_anypen_switch_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

//static ssize_t synaptics_power_hal_suspend_show(struct device *dev,
//        struct device_attribute *attr, char *buf);
//
//static ssize_t synaptics_power_hal_suspend_store(struct device *dev,
//        struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(anypen_switch, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_anypen_switch_show,
			synaptics_rmi4_anypen_switch_store),
};
...
{
	int retval;
	unsigned char pen_sw[4];

	retval = synaptics_rmi4_reg_read(rmi4_data,0X1c,pen_sw,sizeof(pen_sw));
			if (retval < 0)
				return retval;
	if(rmi4_data->anypen_status == 1)
		pen_sw[3]=0x02;
	else 
		pen_sw[3]=0x00;
	printk("synaptics_rmi4_anypen_switch open_switch %d pen_sw[0] %d,pen_sw[1] %d,pen_sw[2] %d,pen_sw[3] %d.\n",rmi4_data->anypen_status,pen_sw[0],pen_sw[1],pen_sw[2],pen_sw[3]);
	retval = synaptics_rmi4_reg_write(rmi4_data,0x1c,&pen_sw,sizeof(pen_sw));
			if (retval < 0)
				return retval;
}
...
//	unsigned int input;
	...
	if (sscanf(buf, "%u", &rmi4_data->anypen_status) != 1)
		return -EINVAL;
//	rmi4_data->anypen_status = input;
	synaptics_rmi4_anypen_switch_op(rmi4_data);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			printk(rmi4_data->pdev->dev.parent,
					"%s: Finger %d: "
					"status = 0x%02x, "
					"x = %d, "
					"y = %d, "
...
printk("synaptics synaptics_rmi4_set_input_dev bdata->swap_axes=%d, max_y=%d max_x=%d.\n",bdata->swap_axes,rmi4_data->sensor_max_y,rmi4_data->sensor_max_x);
...
sysfs_create_link (NULL,&rmi4_data->input_dev->dev.kobj,"hideep");
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry(exp_fhandler, &exp_data.list, link)
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	// Pullup reset pin befor poweron, because BIOS can't init reset pin.
	if (bdata->reset_gpio >= 0) {
		retval = synaptics_rmi4_gpio_setup(
				bdata->reset_gpio,
				true, 1, !bdata->reset_on_state);
	...
	touch_set_pmic_power(true);

	/* differentiate hardware designs by chip_id. */
	retval = synaptics_rmi4_reg_read(rmi4_data,
			0x00EE,
			&chip_id,
			1);
	dev_info(rmi4_data->pdev->dev.parent, "synaptics chip id: 0x%02x \n", chip_id);
	if (retval < 0 || chip_id != 0x34) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read chip id registers[0xEE] = 0x%02x\n",
				__func__, chip_id);
		goto err_get_reg;
	}
...
#ifdef CONFIG_PM_SLEEP
	retval = synaptics_power_hal_suspend_init(&pdev->dev);
	if (retval < 0)
		dev_err(&pdev->dev, "Unable to register for power hal");
	mutex_init(&rmi4_data->power_hal_lock);
#endif
...
#ifdef CONFIG_PM_SLEEP
	retval = sysfs_create_file(&pdev->dev.kobj, &power_attr.attr);
	if (retval < 0) {
		dev_err(&pdev->dev,
				"%s: Failed to create pdev sysfs attributes\n",
				__func__);
		sysfs_remove_file(&pdev->dev.kobj, &power_attr.attr);
	}
#endif
...
#ifdef CONFIG_PM_SLEEP
	synaptics_power_hal_suspend_destroy(&pdev->dev);
#endif
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    return sprintf(buf, "%s\n",
        rmi4_data->power_hal_want_suspend ? "suspend" : "resume");
}
                         ...
                         const char *buf, size_t count)
{
    ...
    dev_info(rmi4_data->pdev->dev.parent, ": %s suspended %d \n", buf, rmi4_data->suspend);

    if (!strncmp(buf, POWER_HAL_SUSPEND_ON,
             POWER_HAL_SUSPEND_STATUS_LEN)) {
        synaptics_power_hal_suspend(dev);
    } else {
        synaptics_power_hal_resume(dev);
    }
...
static struct device_attribute power_attr =
	__ATTR(power_HAL_suspend, (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP),
			synaptics_power_hal_suspend_show,
			synaptics_power_hal_suspend_store);
#endif
...
void synaptics_power_hal_suspend(struct device *dev)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);

	dev_info(rmi4_data->pdev->dev.parent, "synaptics_power_hal_suspend\n");
	mutex_lock(&rmi4_data->power_hal_lock);
	if (rmi4_data->power_hal_want_suspend)
		goto out;
	synaptics_rmi4_early_suspend(dev);
	rmi4_data->power_hal_want_suspend = true;
out:
	mutex_unlock(&rmi4_data->power_hal_lock);
}

void synaptics_power_hal_resume(struct device *dev)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);

	dev_info(rmi4_data->pdev->dev.parent, "synaptics_power_hal_resume\n");
	mutex_lock(&rmi4_data->power_hal_lock);
	if (!rmi4_data->power_hal_want_suspend)
		goto out;
