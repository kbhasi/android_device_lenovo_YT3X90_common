...
#define OFM_FW_IMAGE_NAME "synaptics/Lenovo_Blade3_10.1_Ofilm_DVT2_PR1976849_DS5_9_0_0_1117_01540052_CDM4_20150703.img"
#define GIS_FW_IMAGE_NAME "synaptics/Lenovo_Blade3_DVT3_06410023_PR2302687_20160726.img"
...
static ssize_t synaptics_rmi4_configid_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static enum flash_area fwu_go_nogo(void)
{
	...
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
	fwu->force_update = 1;
	printk("synaptics fwu_go_nogo  fwu->force_update %d.\n",fwu->force_update);
	if (fwu->force_update) {
	...
	if (fwu->image == NULL) {
		...
		if (config_id[0] == 0x01) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
					OFM_FW_IMAGE_NAME, sizeof(OFM_FW_IMAGE_NAME),
					sizeof(OFM_FW_IMAGE_NAME));
		} else if (config_id[0] == 0x06) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
					GIS_FW_IMAGE_NAME, sizeof(GIS_FW_IMAGE_NAME),
					sizeof(GIS_FW_IMAGE_NAME));
		}
		else if (config_id[0] == 0x00) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
					GIS_FW_IMAGE_NAME, sizeof(GIS_FW_IMAGE_NAME),
					sizeof(GIS_FW_IMAGE_NAME));
		}
printk("synaptics : config_id %d fw name %s.\n",config_id[0],fwu->image_name);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_info(rmi4_data->pdev->dev.parent,
			  "configid 0x%02x, 0x%02x, 0x%02x, 0x%02x\n", config_id[0], config_id[1],config_id[2],config_id[3]);

	return snprintf(buf, PAGE_SIZE, "synaptics_%s_%s_0x%02x\n",
			rmi4_data->rmi4_mod_info.product_id_string,
			config_id[0] == 0x01 ? "Ofilm" : "GIS",
			config_id[3]);
}
