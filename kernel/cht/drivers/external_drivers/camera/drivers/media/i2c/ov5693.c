...
#define OV5693_DEFAULT_LOG_LEVEL 1
//#define OV5693_DEFAULT_LOG_LEVEL 10
static unsigned int log_level = OV5693_DEFAULT_LOG_LEVEL;
module_param(log_level, int, 0644);

#define OV5693_LOG(level, a, ...) \
        do { \
                if (level < log_level) \
...
/*---------------------------------------------------------- add by wxc for otp start ----------------------------------------------------------*/
#ifdef OV5693_OTP
#include "ov5693_kobe_otp.c"

u8 ov5693_data[DATA_BUF_SIZE];
...
static int power_up(struct v4l2_subdev *sd)
{
	...
	/* according to DS, 30ms is needed between PWDN and i2c access */
	msleep(30);
				    ...
				    int w, int h)
{
	...
	if (((w == 192) && (h == 160)) && (dev->ov5693_res == ov5693_res_preview)) {
		w = 176;
		h = 144;
	}
			     ...
			     struct v4l2_mbus_framefmt *fmt)
{
	...
	dev_info(&client->dev, "ov5693 resolution index:%d width:%d, height:%d, desc:%s\n",
			dev->fmt_idx, dev->ov5693_res[dev->fmt_idx].width, dev->ov5693_res[dev->fmt_idx].height, dev->ov5693_res[dev->fmt_idx].desc);
	if (dev->fmt_idx == -1) {
...
#ifdef OV5693_OTP
       if (enable) {
               OV5693_LOG(2,"OTP check start-----\n");
               if (!Read_OTP(client))
                       OV5693_LOG(0,"OTP check Success\n");
               else
                       OV5693_LOG(0,"OTP check Failed\n");
       }
...
#ifdef OV5693_OTP
	memset(ov5693_data, 0, DATA_BUF_SIZE);
			...
			const struct i2c_device_id *id)
{
	...
	/* add hardware ID */
	//intel_hardware_info_store(INTEL_HW_INFO_FRONT_CAMERA, "front camera","ov5693", "omnivision", "NULL", "NULL");
	return ret;
out_free:
